...
import { CircleDashed, Lock, Mail, Target, User, X } from 'lucide-react'
...
function AuthModel({ open, onClose }: propType) {
  const [step, steStep] = useState<stepType>("login")
  ...
  const handleSignUp = async () => {
    ...
    try {
     ...
     setErr("")
      ...
      setLOading(false)
      setErr(error.response.data.message ?? "Somethng Want Wrong")
    }
  }
            ...
            >
              <div className='relative w-full max-w-md rounded-3xl bg-white border border-black/10 shadow-[0_40px_100px_rgba(0,0,0,0.35)] p-6 sm:p-8 text-black'>
                ...
                <div className='mb-6 text-center'>
                  <h1 className='text-3xl font-extrabold tracking-widest'>RIDEX</h1>
                ...
                <button className=' w-full h-11 rounded-xl
                  ...
                  <Image src={"/google.png"} alt='google' width={20} height={20} unoptimized />
                  ...
                  {step == "login" && (
                    ...
                    >
                      ...
                      <div className='mt-5 space-y-4'>
                        ...
                        <button className='w-full h-11 rounded-xl bg-black text-white font-semibold hover:big-gray-900 transition flex justify-center items-center' onClick={handleLogin}>{!loading ? "Login" : <CircleDashed size={18} color='white' className='animate-spin' />}</button>

                      </div>
                      <p className='mt-6 text-center text-sm text-gray-500'>Don't have an account? <span onClick={() => steStep("signup")} className='text-black font-medium hover:underline'>Sign Up</span></p>
                    </motion.div>
                  )}
                  {step == "signup" && (
                    <motion.div
                    ...
                    >
                      ...
                      <div className='mt-5 space-y-4'>
                        ...
                        <button className='w-full h-11 rounded-xl bg-black text-white font-semibold hover:big-gray-900 transition flex justify-center items-center' disabled={loading} onClick={handleSignUp} > {!loading ? "Send Otp" : <CircleDashed size={18} color='white' className='animate-spin' />}</button>

                      </div>
                      <p className='mt-6 text-center text-sm text-gray-500'>Already have an account? <span onClick={() => steStep("login")} className='text-black font-medium hover:underline'>Login</span></p>
                    </motion.div>
                  )}
                      ...
                      <div className='mt-6 flex justify-between gap-2'>
                        {otp.map((digit, i) => (
                          <input key={i} id={`otp-${i}`}
                            ...
                            rounded-xl bg-white
                            border border-black/20
                            outline-none'onChange={(e) => handleChangeOtp(i, e.target.value)}
                          />
